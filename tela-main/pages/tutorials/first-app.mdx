...
keywords: "TELA first app, TELA tutorial, first TELA app, DERO development, XSWD setup, deploy to blockchain, TELA development, beginner tutorial"
date: "2024-08-26"
lastUpdated: "2025-10-22"
authors: ["DHEBP"]
slug: "tutorials/first-app"
canonicalUrl: "https://tela.derod.org/tutorials/first-app"
tags: ["tutorial", "first-app", "getting-started", "TELA", "DERO"]
---
...
Complete step-by-step tutorial to build your first TELA application - from setup to deployment.
...
**CRITICAL: Name Your Entry File `index.html`**

Your main HTML file **MUST** be named `index.html` for TELA browsers to render it correctly. If you name it anything else (like `myapp.html` or `hello-tela.html`), users will see a file listing instead of your application.

For this tutorial, we'll create `index.html` (not `hello-tela.html`) so it works properly when deployed.
</Callout>
...
({
    ...
    connectXSWD: function() {
        ...
        try {
            ...
            // Application identification (match app1 demo format)
            var appData = {
                "id": "hello-tela-app-v1",
                "name": "Hello TELA",
                "description": "My first TELA application",
                "url": "http://localhost:" + location.port  // Must include port (app1 pattern)
            };
            
            // Create WebSocket connection (use localhost to match app1 demo pattern)
            self.socket = new WebSocket("ws://localhost:44326/xswd");
...
1. **[Advanced Features](/advanced-features/libraries-overview)** - DocShards, compression, and libraries
2. **[Design Reference](/design-reference)** - Professional UI patterns
3. **[EPOCH Mining](/epoch-mining)** - Add mining functionality
4. **[Complete API Guide](/api-reference/complete-api-guide)** - Explore all available methods
...
### Essential Reading
1. **[TELA-CLI Guide](/tela-cli/installation)** - Command-line interface
2. **[Complete API Guide](/api-reference/complete-api-guide)** - All available methods
3. **[Best Practices](/best-practices)** - Deployment and optimization guide
...
<Callout type="info" emoji="🎓">
  **Learning Path**: Start with this tutorial, then explore the [Best Practices](/best-practices), [Complete API Guide](/api-reference/complete-api-guide), and finally dive into [Advanced Features](/advanced-features/libraries-overview) when you're ready to build more complex applications.
</Callout>
...
- [Official Demo (app1)](/demo) - Complete source code from verified working application
- [XSWD Protocol](/xswd) - Complete connection and protocol reference
- [Application Data Handshake](/demo/application-data) - Application data format details
