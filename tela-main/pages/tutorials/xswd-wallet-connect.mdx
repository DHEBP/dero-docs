...
2. Go to **Settings** → **XSWD** (or **Advanced** → **XSWD**)
...
- Engram: `ws://localhost:44326/xswd` ✅ **Matches official app1 demo**
...
10. **ASCII only for `name` and `description`** - Unicode characters will cause rejection

<Callout type="error" emoji="🚨">
**ASCII Only Requirement:** The `name` and `description` fields must contain only ASCII characters (bytes 0-127). Unicode characters like smart quotes (`"` `"`), emojis, or non-English characters will cause the wallet to reject the connection with "Invalid name" or "Invalid description". This is enforced in the DERO wallet's XSWD server code.
</Callout>

### **App ID Generation Options**

The App ID is essentially arbitrary — any valid 64-character hexadecimal string works. The XSWD server only validates length and hex format, nothing more.

**Option 1: Random generation (simple)**
```javascript
const appId = Array.from({length: 64}, () => 
    Math.floor(Math.random() * 16).toString(16)
).join('');
localStorage.setItem('xswd_app_id', appId); // Store for reuse
```
...
**Option 3: Static ID (hardcoded)**
```javascript
const appId = "71605a32e3b0c44298fc1c549afbf4c8496fb92427ae41e4649b934ca495991b";
```

All approaches are valid — choose based on your preference. The key is to **reuse the same ID** across sessions to maintain permission continuity.

<Callout type="info" emoji="ℹ️">
**Can App IDs be duplicated?** Yes. Nothing prevents someone from using the same App ID as another app. The wallet only checks: is it 64 hex chars, is it valid hex, and is this ID already connected in the current session. For TELA applications, the **SCID (Smart Contract ID) is the true identity**, not the applicationData.
</Callout>
...
- **Learn more:** [XSWD Protocol Overview](/xswd)
