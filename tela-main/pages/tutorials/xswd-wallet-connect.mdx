...
- ✅ Page served via HTTP (not `file://` URLs)

<Callout type="warning">
**Important:** The code examples in this tutorial require your page to be served via HTTP (e.g., `http://localhost:8080`). Opening HTML files directly as `file://` URLs will not work because:
- `location.port` is empty for `file://` URLs
- The wallet validates that the `url` field in application data matches the page origin
- TELA apps are deployed via HTTP, so this matches real-world usage

**For local testing:** Use a local HTTP server (e.g., `python3 -m http.server 8080`).
</Callout>
...
9. **URL must match origin** - Use `"http://localhost:" + location.port` for app data. **Must be served via HTTP** (not `file://` URLs) - `location.port` is empty for `file://` URLs, causing origin mismatch errors.
10. **Requirements for code to work:**
    - ✅ Wallet running with XSWD enabled
    - ✅ Page served via HTTP (not `file://`)
