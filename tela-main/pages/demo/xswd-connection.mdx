...
**ASCII Only Requirement:** The `name` and `description` fields in application data must contain only ASCII characters (bytes 0-127). Unicode characters like smart quotes, emojis, or non-English characters will cause the wallet to reject the connection with "Invalid name" or "Invalid description". This is enforced by the DERO wallet's XSWD server.
</Callout>
...
| Unicode in name/description | "Invalid name" or "Invalid description" error | Use ASCII only (no emojis, smart quotes, etc.) |
