...
## x402 Payment Guard (How It Works)

The x402 guard lets you charge for API access using standard HTTP `402 Payment Required` semantics.

In practice, it works in three steps:

1. A client requests a protected route.
2. If no valid receipt is present, the server responds with a machine-readable `402` payment challenge.
3. After payment is confirmed and a receipt is issued, the client retries with that receipt and gets the resource.
...
### What the Client Sends on Retry

After payment, retry with either:

- `X-DeroPay-Receipt: <token>`
- `Authorization: X402 proof="<token>"`

Receipts are short-lived by default (`600` seconds) when issued via `issueReceiptHandler`, and you can override that with `ttlSeconds`.

### Why This Is Fast and Safe

The guard verifies signed receipts locally instead of doing chain verification on every protected request. It also supports:

- short-lived receipt expiry
- resource binding (`/report` receipt cannot unlock `/inference`)
- optional single-use replay protection with `jti`
- key rotation with `kid` + keyring verification
- route-level quotas (`maxReceiptsPerDay`, `maxAtomicPerWindow`)
- structured `x402Audit` events for monitoring

For multi-instance deployments, use a shared persistent store so quota and replay checks stay consistent across instances.

### Related Handlers

Expose receipt issue/verify endpoints:
