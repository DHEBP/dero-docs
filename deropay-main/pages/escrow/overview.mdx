...
<Callout type="info">
  **Refund safety is verified on mainnet.** In an end-to-end test on DERO mainnet (block 7,348,673), a rejected/underpaid deposit refunded the buyer and left the escrow box fund-safe and reusable. That test exercises the reject/underpay path specifically — strong evidence for it, not a blanket guarantee for every failure mode.
</Callout>
