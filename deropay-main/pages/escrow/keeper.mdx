...
const manager = new EscrowManager({
  ...
  escrowInventory: new SqliteEscrowInventoryStore("./escrow-inventory.db"),
...
| Store | Use |
|---|---|
| `MemoryEscrowInventoryStore` | Development / tests, single process only |
| `SqliteEscrowInventoryStore` | Production — pops boxes atomically across processes |
...
**Confirm gate.** A minted box becomes pool-ready **only** once `GetSC` proves it reads back as `owner set` + `bound == 0` + `status == 0` — empty and ours. Anything else stays `minted` and is retried next tick, so an unconfirmed SCID is never handed to a checkout (binding one would fail with "SC not found").
...
const inventory = new SqliteEscrowInventoryStore("./escrow-inventory.db");
