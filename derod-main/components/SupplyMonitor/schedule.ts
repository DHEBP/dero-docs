 ...
 * from the fixed emission schedule — no node is contacted. Both SupplyMonitor
 * and SupplyRibbon import from here so the height anchor lives in ONE place.
 *
 * ANCHOR: refresh ANCHOR_HEIGHT/ANCHOR_TIME each deploy to bound drift.
 *
 * Emission constants (DHEBP/derohe + canonical deroproject/derohe):
...
// Terminal / true finite max supply = 20,890,694 DERO. Reward halves each RRI and
// floors to 0 near block ~112M — a real cap. This is the sum of every FLOORED epoch
// reward (61500, 30750, …, 1) × RRI + premine, ~560 DERO below the naive
// premine + BASE*RRI (which ignores integer-floor truncation).
export const TERMINAL_ATOMS = 2_089_069_400_000; // = 20,890,694 DERO
...
// Total atomic supply at (possibly fractional) height hF:
// premine + sum of CalcBlockReward(h) for h in [0, floor(hF)) + partial block.
