...
description: "Run a read-only, agent-ready DERO MCP server — stdio for local hosts, streamable-HTTP for remote agents. 21 daemon primitives + 12 composite tools (incl. TELA app inspection and dURL discovery) that fuse live chain reads with the bundled DERO docs index, plus structured errors and curated citations — no middlemen, no API keys, no surveillance."
...
| Surface | Count | Notes |
|---|---:|---|
| **Tools** | **33** | 18 daemon RPC wrappers + 3 docs-index tools + 12 composites |
...
| ↳ Composite tools | 12 | Multi-step intent-shaped flows that fan out into primitives + docs and return a narrative with curated citations — including supply verification (`verify_supply`) and a full TELA suite (inspect, read file content, discover apps by dURL) |
...
AI assistants are powerful research and dev tools, but most blockchain integrations route through centralized APIs that log your queries. This server runs on **your machine**, talks to **your daemon** (or a public one you choose), and communicates with your AI over **local stdio**. No accounts, no analytics, no tracking — just code you can audit. The bundled DERO documentation index (151 pages across `derod`, `tela`, `hologram`, and `deropay`) is shipped inside the package, so the AI can quote primary sources offline without a single network round-trip beyond the daemon you pointed it at.
...
Requires Node.js 20+. Source code: [github.com/DHEBP/dero-mcp-server](https://github.com/DHEBP/dero-mcp-server) · npm: [dero-mcp-server](https://www.npmjs.com/package/dero-mcp-server)
...
|---|---|---|
| `verify_supply` | Offline CalcSupply + optional `GetInfo.total_supply` cross-check | "What is total supply / does the node match the schedule" — recomputes premine + launch credit + epoch rewards; cites [Verify the Supply](/integrity/verify-the-supply). Schedule-only (not a UTXO census) |
...
<Callout type="info">
**`verify_supply` and GetInfo.** The tool always recomputes total supply from the emission schedule (CalcSupply). `GetInfo.total_supply` is whatever formula that daemon build implements. Some public nodes still expose an older linear schedule readout that diverges after halvings; a `match: false` there means the **RPC field differs from CalcSupply**, not that coins appeared or disappeared. For a same-build cross-check, point the server at a current node or pass `height` and treat the offline number as authoritative. Details: [Verify the Supply](/integrity/verify-the-supply).
</Callout>
...
| `dero://mcp/composites` | Structured catalog of all 12 composites — what each replaces, when to call it, what it returns, and which `_meta.error` codes it can emit |
...
Every one of the 33 tools carries the MCP read-only annotation block:
...
- Agent-discovery surfaces at [`/.well-known/agent-card.json`](/.well-known/agent-card.json) (A2A v0.2-draft descriptor for the broader agent ecosystem) and [`/.well-known/mcp-server-card.json`](/.well-known/mcp-server-card.json) (a compatibility document, not an official MCP Registry schema)
...
- [Running a DERO Node](/basics/running-a-node) — set up your own daemon
