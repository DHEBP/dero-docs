...
`SIGNER()` returns the wallet address that sent the current transaction. **It does not mean "the owner of this contract."** If your contract has no explicit owner check, anyone can call `UpdateCode` and overwrite your logic — the DVM does not gate `UpdateCode` at the protocol level.

<CaptainNote
  variant="verified"
  source="https://discord.com/channels/419781880632836096/737351309396541471/813933195850416168"
  date="2021-02-24"
  channel="#dero-he-testnet"
  codeRef="dvm/dvm_functions.go (dvm_signer, dvm_update_sc_code — // TODO verify code authenticity how)"
>
  SIGNER() -&gt; its the wallet adress which is sending this TX. It may not be the owner of the SC. Please install owner address in DB during SC install. If you have no owner stored &amp; verification anyone can update your SC.
</CaptainNote>
