...
>
  SC download/balance/parameters checks are done on blockchain using daemon port. But sending/SC install/update/balance transfer etc. are done using wallet port by rpc methods.
</CaptainNote>
...
<CaptainNote
  variant="verified"
  source="https://discord.com/channels/419781880632836096/737351309396541471/914341300826963968"
  date="2021-11-28"
  channel="#dero-he-testnet"
  codeRef="dvm/sc.go (SC_Code_Key = 'C'); cmd/derod/rpc/rpc_dero_getsc.go (zerohash native-DERO balance, hex-encoded SCID keys in balances map)"
>
  The `C` key holds the contract source as hex. The `balances` map is keyed by SCID; `0x000...000` is the native DERO asset.
</CaptainNote>
...
<CaptainNote
  variant="verified"
  source="https://discord.com/channels/419781880632836096/737351309396541471/914510084900536350"
  date="2021-11-28"
  channel="#dero-he-testnet"
  codeRef="walletapi/rpcserver/rpc_websocket_server.go:208 (install_sc), :319 (scinvoke); walletapi/rpcserver/rpc_scinvoke.go:28"
>
  Three steps: install_sc on wallet port, getsc on daemon port, scinvoke on wallet port.
</CaptainNote>

Captain's original used testnet ports (`40402`/`40403`). The examples above are converted to mainnet. The RPC shapes (`sc_dero_deposit`, `sc_rpc` array, `ringsize`) are unchanged on Release 142.
