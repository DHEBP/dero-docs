...
- When **9** shows non-zero seconds, your clock is drifting — fix NTP (see [Mining → time sync](/basics/mining)).
...
The `status` command's `Block Minted` line is `(MB + IB) − MBR` — so when `MBR` climbs, your minted count visibly drops. If `MBR` keeps growing, you're probably running an unofficial miner that submits malformed work; switch back to the official `dero-miner` binary.
...
If you need full history (e.g. to serve TX queries from genesis), restart `derod` without `--fastsync` and let it backfill — Captain's recommended path is a one-time full sync after the initial fastsync onboard.
