...
|---|---|---|
| P2P | `10101` | **Randomized at startup** — check `derod` log for actual port, or pin with `--p2p-bind=0.0.0.0:10101` |
...
<CaptainNote
  ...
  date="2023-04-30"
  channel="#dero-mining"
  codeRef='p2p/controller.go:502 (default_address := "0.0.0.0:0" — random port; --p2p-bind override on :503-516); README.md:133 (documented 10101 default)'
>
...
|---|---|---|
| `--fastsync` | Skip historical block bodies during initial sync; download recent state snapshots instead. | One-time onboarding to get your node online quickly. **Bootstrap-only flag** — it's a no-op after first sync, so you can leave it on or drop it; either way you'll keep validating new blocks normally. |
...
<CaptainNote
  ...
  date="2023-04-13"
  channel="#dero-mining"
  codeRef="cmd/derod/main.go:62,73,81 (docopt flag declarations + bootstrap-only semantics); blockchain/blockchain.go:268-269 (chain.Sync toggled from --fastsync)"
>
...
<CaptainNote
  ...
  date="2023-04-08"
  channel="#dero-mining"
  codeRef="config/seed_nodes.go:23,36 (Mainnet_seed_nodes / Testnet_seed_nodes); p2p/controller.go:265-270 (bootstrap-only seed dial); :431-434 (random seed pick on rebootstrap)"
>
