...
<CaptainNote
  variant="verified"
  source="https://discord.com/channels/419781880632836096/737351309396541471/946298626005672016"
  date="2022-02-24"
  channel="#dero-he-testnet"
  codeRef="walletapi/transaction_build.go (per-ring-member ElGamal commitment loop) — every commitment is blinded uniformly with the same random r, so sender (−value), receiver (+value), and decoy (0) commitments are indistinguishable regardless of ring size"
>
  Whatever the RingSize is set in DERO amounts are never revealed.
</CaptainNote>

This is why [ring size 2](/privacy/ring-signatures#why-ring-size-2-offers-no-anonymity-set) — which removes *sender* anonymity by design — still leaves the *transferred amount* fully encrypted. The two privacy properties are decoupled.
...
<CaptainNote
  variant="verified"
  source="https://discord.com/channels/419781880632836096/419781880632836098/1050433141875155064"
  date="2022-12-08"
  channel="#dero-discussion"
  codeRef="cryptography/crypto/algebra_elgamal.go (66-byte ElGamal codec — 33B Left ‖ 33B Right compressed G1 points); walletapi/daemon_communication.go (GetEncryptedBalanceAtTopoHeight)"
>
...
This is why DERO wallets open in seconds while traditional privacy-coin wallets need to scan the chain for minutes — there is no chain to scan, only a single 66-byte ciphertext to fetch and decrypt locally.
