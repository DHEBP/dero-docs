...
- Layered privacy (TLS + anonymity set + homomorphic encryption) with a single verifier-enforced integrity gate (bulletproof soundness + balance proofs)
...
- Bulletproof soundness rejects any committed value outside the proven range — including the `uint64` wraparound of a negative — at the verifier, on every node.
...
- [Private Smart Contracts](/privacy/private-smart-contracts) - Contract code and state variables are PUBLIC; token balances **in user wallets** stay encrypted, transferable peer-to-peer without the contract seeing the amount
