...
The ambiguity that protects sender privacy comes at a cost: **any ring member can generate a verifying payload proof for the transaction.** This is not a defect — it is the visible consequence of how ring signatures work. The commitment check an explorer performs (`amount × G + address`) matches for every ring member by construction, because all ring members were constructed using the same amount (see [Why All Ring Members Use Same Amount](#why-all-ring-members-use-same-amount) below). If only the real sender could produce a verifying proof, ring-signature privacy would not exist.
