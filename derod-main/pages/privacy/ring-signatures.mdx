...
The proof generation encodes this split into two separate polynomial sets — `P` for even-indexed ring members, `Q` for odd-indexed ring members.
...
With ring size 2: one even slot (index 0), one odd slot (index 1). The sender occupies one slot, the recipient the other.

Ring size 2 provides no meaningful anonymity — an observer sees two members, one sender and one recipient, and any out-of-band signal (timing, IP, repeated patterns across transactions, known counter-party relationships) resolves the pair. Do not rely on it for privacy.
