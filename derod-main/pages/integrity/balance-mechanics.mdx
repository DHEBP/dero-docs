...
func (e *ElGamal) Serialize() []byte {
    // Returns 66 bytes total
    return append(e.Left.EncodeCompressed(), e.Right.EncodeCompressed()...)
}
```
...
**For context:**
- Block 144,000 ≈ 30 days after mainnet launch
- Current block height: 6,000,000+
- 99.9%+ of addresses have zero initial balance

<Callout type="warning" emoji="📅">
**Timeline:** The 200-unit bonus only applied to addresses registered in the first month of mainnet. Any address registered after block 144,000 (October 2020+) starts with exactly zero DERO.
</Callout>
...
<Tabs items={['Encrypted (Anyone)', 'Decrypted (Owner Only)']}>
  ...
  <Tabs.Tab>
    **Decrypt Balance (Requires Private Key)**
    ...
    ```go
    // Only wallet with private key can decrypt
    func (w *Wallet) DecryptBalance(encrypted []byte) uint64 {
        elgamal := crypto.DeserializeElGamal(encrypted)
        
        // L - x*R = amount*G
        // Use baby-step giant-step to solve DLP
        amount := w.DecryptAmount(elgamal)
    ...
    **Who can do this:** Only the address owner
  </Tabs.Tab>
</Tabs>
...
```go
// Simplified from blockchain/transaction_execute.go

func VerifyConservation(tx *Transaction) bool {
    // Sum of encrypted inputs
    inputSum := SumEncrypted(tx.Inputs)
    
    // Sum of encrypted outputs
    outputSum := SumEncrypted(tx.Outputs)
    
    // Must be equal (homomorphic check)
...
**Always Encrypted:** From creation to storage to operations, DERO balances are always encrypted. The network processes billions in value without ever seeing a single plaintext balance.
</Callout>
