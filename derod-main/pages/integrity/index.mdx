...
| A_t encodes a combined 128-bit value: 64 bits transfer + 64 bits remaining. Bulletproof construction starts here | [Range Proofs](/integrity/range-proof-integrity) |
...
| Bulletproof soundness rejects any committed value outside the proven range — including negative/wraparound amounts | [Negative Transfers](/integrity/negative-transfer-protection) |
...
| What Happens | Learn More |
|-------------|------------|
| Total supply is public and schedule-verifiable — recompute it offline and audit it yourself | [Verify the Supply](/integrity/verify-the-supply) |
...
| October 2022 inflation claim: claims vs. evidence | [Inflation Claim](/integrity/inflation-claim) |
...
| Threat | Protection |
|--------|------------|
| **Negative transfers** | Bulletproof soundness (verifier-enforced range) |
...
|------------|------|-------|
| `Verify()` entry point | `cryptography/crypto/proof_verify.go` | 98 |
| Overflow check | `cryptography/crypto/proof_verify.go` | 108-111 |
| Parity check | `cryptography/crypto/proof_verify.go` | 136-141 |
| Challenge hash | `cryptography/crypto/proof_verify.go` | 410-425 |
| Inner product | `cryptography/crypto/proof_verify.go` | 457-460 |
