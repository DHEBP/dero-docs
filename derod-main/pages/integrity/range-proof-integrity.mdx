...
DERO packs **two** quantities into a single 128-bit value and range-proves them together (`cryptography/crypto/proof_generate.go:468-471`):
...
If the final inner product does not match the committed value, the check at `proof_verify.go:457-459` fails — and the transaction is rejected at consensus.
