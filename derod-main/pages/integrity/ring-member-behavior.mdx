...
func AmITheSender(tx *Transaction) bool {
    before := wallet.GetDecryptedBalance(tx.Block - 1)
    after := wallet.GetDecryptedBalance(tx.Block)
    
    if before != after {
        return true  // I was the sender
    }
    return false  // I was a decoy
}
```
