...
flowchart TD
    ...
    E --> F["Blockchain sees:<br/>ONE of 8 possible senders sent it"]
...
flowchart TD
    ...
    subgraph "With Balance Change (DERO)"
        ...
        A2 --> B2["Attacker knows:<br/>One of 8 possible senders is the real sender"]
...
**Legal Protection:** If accused of sending a transaction, you can truthfully say: "My encrypted balance changed, but that only means I was in the ring. Any of the 8 potential senders in this ring could be the real sender." This is cryptographically provable.
</Callout>
...
Transaction at block 1,081,893:
  Ring size: 16 members (8 sender slots, 8 recipient slots)
  One of the 8 possible senders sent 100 DERO
```
...
Which of the 8 possible senders is the real sender? UNKNOWN.
```
...
<Callout type="info" emoji="📐">
**Mathematical Privacy:** The probability of correctly identifying the sender is exactly 1/(ring_size/2) (e.g., 12.5% for ring size 16). Rings are split evenly — half are potential senders, half are potential recipients — so the anonymity set for the sender is ring_size/2. This is not "hard to guess" - it's mathematically guaranteed randomness.

**Source:** This split is enforced by a parity constraint in `walletapi/transaction_build.go:79-90` and by the P/Q polynomial structure in `cryptography/crypto/proof_generate.go:700-710`. See [The Parity Constraint](/privacy/ring-signatures#the-parity-constraint-how-the-ring-is-split) for the full source-code-verified breakdown.
</Callout>
...
- **Statistical analysis** faces 1/(ring_size/2) probability barrier (e.g., 1/8 for ring size 16)
