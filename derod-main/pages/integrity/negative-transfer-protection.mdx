...
**Cryptographic impossibility:** A negative — or `uint64`-wraparound — transfer amount cannot produce a transaction that passes DERO consensus. The guarantee comes from the **soundness of the Bulletproof range proof** combined with **homomorphic value conservation** — not from a string check that could be patched out of a wallet.
</Callout>
...
The decisive property is **soundness**. A Bulletproof convinces the verifier that the committed value is in range *without revealing it* — and soundness means a prover **cannot construct a proof that verifies** for a value outside the range. Forging one is as hard as breaking the discrete-log assumption the curve rests on. There is no "almost in range," no rounding, no edge case: either a verifying proof exists (value in range) or it does not.
...
The same holds for the alleged October 2022 inflation: a negative-transfer mint cannot reach consensus.
