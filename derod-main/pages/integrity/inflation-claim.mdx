...
<Callout type="info" emoji="🔬">
**Even the on-screen amount's last digit is display-only.** Decode the bech32 string as an integer (`rpc.NewAddress` → `args.Value(RPC_VALUE_TRANSFER, DataUint64)`) and the embedded value is `18,446,743,853,709,551,435` — exactly **−2,200,000.00181 DERO**, the report's own stated figure. The explorer prints `.51434` only because `FormatMoney` divides that `uint64` in a **64-bit-mantissa `big.Float`** (`FormatMoneyPrecision`), whose step size near 10¹⁴ (~`0.0000153 DERO`) is *just over* one atomic unit (`0.00001 DERO`). Because the gap is barely more than a single atom, the rounding error lands in the **last** place — `…51435` renders as `…51434`. An "off-by-one" read of the display is reading that rounding, not the integer in the string, which matches the report to the atom. A layer that cannot reliably render its own last digit is not consensus state.
</Callout>
...
// pubkey is prepended so the receiver can still derive the shared secret.
ephemeral_scalar := /* SHAKE-derived from sender + roothash + ring + receiver */
