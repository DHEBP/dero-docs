...
<Callout type="warning" emoji="⚖️">
**What `GetInfo` can and cannot do:** Comparing `GetInfo.total_supply` to the emission formula confirms the RPC implements the schedule. It does **not** independently detect a surreptitious mint — both numbers come from the same model. DERO does not expose a separately-computed chain-wide balance sum via RPC, but it doesn't need to: **the proof system itself enforces supply conservation**. Every accepted transaction is balance-preserving (inputs and outputs are bound by the sigma proofs to balance exactly), so the cumulative supply at any height equals premine plus the sum of block rewards **by construction** — regardless of whether the RPC exposes a balance-sum endpoint. The **independent** checks for this case are: block-header **reward**, cited-TX **acceptance** (range proofs passed), and the **impossibility** of the alleged negative-transfer mechanism ([above](#negative-transfers-fail-range-proof-validation)).
</Callout>
