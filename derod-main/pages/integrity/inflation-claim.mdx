...
For **today's** supply, `getinfo.total_supply` is a convenience estimate that **varies by node software** — canonical nodes return the linear number; the [DHEBP fork](https://github.com/DHEBP/derohe) returns `CalcSupply()` (in `blockchain/supply.go`), which sums each reward epoch **and adds the fork's fixed launch claim credit** (271,739,600 atomic = 2,717.396 DERO, a one-time 0.002 DERO credit to each account registered before block 144,000), so it reports higher than both the linear number and the pure epoch sum. Neither is ground truth; the authoritative number is recomputable offline from the deterministic schedule and is identical for everyone. To verify the current supply, see [Verify the Supply](/integrity/verify-the-supply).
</Callout>

<Callout type="info" emoji="📊">
**Burden of proof.** Supply conservation isn't audited — it's built in. Every accepted transaction is bound by range proofs (amounts must be non-negative) and balance proofs (inputs must equal outputs), so **no transaction can ever add to the supply** — cumulative supply is fixed by the launch constants plus **scheduled block rewards** alone (premine, plus a fork's fixed launch claim credit if present — a one-time constant closed at block 144,000, never a mint). `GetInfo.total_supply` reports that schedule, not a UTXO census — and it doesn't need to.
