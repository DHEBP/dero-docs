...
| Does the deanonymization support the ~2.2M? | **No — it refutes it.** The figure is the forgeable proof string, not a deanonymization output. Read the chain directly and what you see is an **ordinary transfer of coins the sender already held**. [Part 5](#part-5-what-deanonymization-can-and-cannot-establish) |
...
| *"Sender actually received 2.2M DERO"* | The ~2.2M is the [forgeable proof string](#part-3-forge-a-fake-proof-yourself), not a deanonymization result. Read the chain directly and what you see is an **ordinary transfer of coins the sender already held**. [Part 5](#part-5-what-deanonymization-can-and-cannot-establish) |
...
<Callout type="info" emoji="🤖">
Live now — `npx dero-mcp-server` (v0.4.0+) or the hosted endpoint at `mcp.derod.org/mcp`, both shipping the forge tool and local-first node resolution ([source](https://github.com/DHEBP/dero-mcp-server)).
</Callout>
...
Scheduled emission is **deterministic** from block height: premine plus block rewards that halve on a fixed schedule (`CalcBlockReward` in `blockchain/transaction_execute.go`). That formula estimates *expected* cumulative supply at any height. Canonical **`DERO.GetInfo` reports a linear approximation of it** (`cmd/derod/rpc/rpc_dero_getinfo.go:81-82`: `PREMINE + CalcBlockReward(topoheight) × topoheight`, then divided by `100,000` to convert atomic units to whole DERO) — scheduled emission in whole DERO, **not** a UTXO census or balance sum across the ledger.

<Callout type="info" emoji="🎯">
**Why the linear formula is exact here.** `PREMINE + CalcBlockReward(topoheight) × topoheight` prices *every* block at the current reward. That is **exact only before the first halving** (block **7,000,000**) — where every block truly paid the same 0.615 DERO. Topo **1,081,893** is pre-halving, so the linear number is the true scheduled supply to the atom, and this rebuttal holds exactly. **After** the first halving it **undercounts** (it charges the ~7M epoch-0 blocks the lower post-halving reward).

For **today's** supply, `getinfo.total_supply` is a convenience estimate that **varies by node software** — canonical nodes return the linear number; the [DHEBP fork](https://github.com/DHEBP/derohe) returns `CalcSupply()` (in `blockchain/supply.go`), which sums each reward epoch **and adds the fork's fixed launch claim credit** (271,739,600 atomic = 2,717.396 DERO, a one-time 0.002 DERO credit to each account registered before block 144,000, [detailed here](/integrity/verify-the-supply)), so it reports higher than both the linear number and the pure epoch sum. Neither is ground truth; the authoritative number is recomputable offline from the deterministic schedule and is identical for everyone. To verify the current supply, see [Verify the Supply](/integrity/verify-the-supply).
</Callout>

<Callout type="info" emoji="📊">
**Burden of proof.** Supply conservation isn't audited — it's built in. Every accepted transaction is bound by range proofs (amounts must be non-negative) and balance proofs (inputs must equal outputs), so **no transaction can ever add to the supply** — cumulative supply is fixed by the launch constants plus **scheduled block rewards** alone (premine, plus a fork's fixed launch claim credit if present — a one-time constant closed at block 144,000, [detailed here](/integrity/verify-the-supply), never a mint). `GetInfo.total_supply` reports that schedule, not a UTXO census — and it doesn't need to.
...
Steps 1–2 are the independent checks. Step 3 shows `GetInfo.total_supply` is a schedule estimate, **not** a UTXO census — it does not detect a surreptitious mint. On a canonical node the readout is the linear formula (`PREMINE + CalcBlockReward × height`), exact at this pre-halving block; on a [fork](https://github.com/DHEBP/derohe) it is `CalcSupply()` — the epoch-summed schedule plus the fork's fixed launch claim credit — so the exact number varies by node software (see [Verify the Supply](/integrity/verify-the-supply)).
...
# --- Optional: compare GetInfo to the linear schedule (not a mint audit) ---
# Exact ONLY before the first halving (block 7,000,000) and on a canonical node.
# Post-halving the linear readout undercounts; a fork node (CalcSupply) reads higher.
# For today's supply see /integrity/verify-the-supply.
def getinfo_matches_schedule(topoheight: int, getinfo_supply: int) -> None:
    ...
    print(f"  Linear schedule: {scheduled:,.3f} DERO")
    print(f"  GetInfo reports: {getinfo_supply:,} DERO (whole DERO)")
    if topoheight < REWARD_REDUCTION_INTERVAL:
        print(f"  Pre-halving: linear is exact — a match confirms the RPC uses the schedule.")
    else:
        print(f"  Post-halving: linear UNDERCOUNTS; readout varies by node software (canonical linear vs fork CalcSupply).")
