...
![Offline-First Browsing](/assets/offline.png)
...
1. **Favorite apps** in the Browser's app discovery
...
Browse TELA apps in the [TELA Browser](/browser) and click the heart icon to add them to your favorites. Favorites are stored locally and persist across sessions.
...
BatchPrefetchFavorites(favorites, minRating) → {
    ...
    failed: 0,
    duration_ms: 2340,
    results: [ { scid: "abc...", status: "prefetched", name: "...", version: 3 } ]
}
```
...
DiffCachedVsOnChain(scid) → {
    ...
    onchain_size: 9540,
    diff: [
        { type: "added", line: 15, content: "const newFeature = true;" },
        { type: "removed", line: 22, content: "// old comment" }
    ]
}
```
