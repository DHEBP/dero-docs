...
Hologram analyzes the XSWD methods your app uses and automatically determines the appropriate permission level:

#### Read-Only Detection

If your app only calls daemon methods (no wallet access needed):
- **No wallet required** - Users can approve without opening a wallet
- **"Read-Only Access" badge** - Clear indication of limited permissions
- **Friendly description** - "This app only reads public blockchain data"
- **No scary warnings** - No misleading wallet permission requests

#### Wallet Permission Detection

If your app uses wallet methods:
- **Full permission dialog** - Shows what wallet access is requested
- **Explicit approval** - User must approve wallet connection
- **Clear indicators** - Shows what's allowed (✓) and denied (✗)
...
### Permission Scopes (HOLOGRAM Extension)

<Callout type="warning">
  **Note**: This is a HOLOGRAM-specific extension to the XSWD protocol. The official DERO XSWD uses per-method permissions with signatures. HOLOGRAM's scope-based system is simpler but only works in Hologram.
</Callout>
...
|-------|---------|-------------|-----------------|
| `read_public_data` | `DERO.GetInfo`, `DERO.GetBlock`, `DERO.GetTransaction`, `DERO.GetSC`, etc. | Read-only blockchain data | **No** |
...
By default, apps are treated as **read-only**:

- **No permissions requested** - Apps start with `read_public_data` only
- **Explicit requests** - Apps can include `permissions` array in handshake to request specific scopes
- **Backward compatible** - Existing apps without permission requests work as read-only
