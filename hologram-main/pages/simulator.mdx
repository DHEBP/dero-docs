...
<Callout type="success">
  **One-Click Setup**: Once the simulator binary is in place, Hologram starts the daemon, creates wallets, mines test coins, and configures Gnomon—all with a single click. The simulator daemon is a separate binary (not the regular `derod`); build it with `make all` or download it from DERO releases if Hologram reports it's missing.
</Callout>
...
1. Verifies the simulator binary is present (errors with build/download instructions if missing)
2. Starts daemon in simulator mode
3. Creates/opens the pre-seeded simulator wallets (empty password, matching the original DERO simulator)
4. Mines initial blocks for test DERO
5. Configures Gnomon for simulator network

<Callout type="info">
  Simulator wallets use an empty password (matching the original DERO simulator) because security isn't needed for test coins.
</Callout>
...
The simulator wallet is auto-managed during `StartSimulatorMode`:

```go
// Check simulator wallet state
GetSimulatorWalletStatus() -> {
    success: true,
    isSetup: true,
    walletCount: 22,
    primaryAddress: "deto1...",
    primaryBalance: 100000000000000,
    primaryLocked: 0,
...
<Callout type="info">
  Simulator Mode automatically creates/opens a wallet and mines initial blocks for test DERO during startup. No manual wallet or mining steps are needed.
</Callout>
...
<Callout type="info">
  All test wallets use an empty password (matching the original DERO simulator). Click any wallet to instantly open it.
</Callout>
...
GetSimulatorTestWallets() -> {
    ...
    count: 22,
    wallets: [
        { index: 0, address: "deto1...", balance: 1000000000000, registered: true, rpcPort: 0 },
        ...
    ]
}

// Sync balances from daemon
SyncSimulatorTestWallets() -> { success: true, count: 22, wallets: [...] }

// Open a specific test wallet (index is 0-based)
OpenSimulatorTestWallet(index int) -> {
    success: true,
    address: "deto1...",
...
2. **Test edge cases**: Use pre-seeded test wallets with various balances for different scenarios
