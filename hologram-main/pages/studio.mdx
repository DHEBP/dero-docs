...
3. **Shard INDEX created** — A special INDEX (dURL ending in `.shards`) links all chunks in order
...
ShardFile(filePath string, maxSize int) -> {
    success: true,
    shards: [
        { index: 0, content: "...", size: 16384 },
        { index: 1, content: "...", size: 12288 },
    ],
    totalShards: 2,
    originalSize: 28672,
}

// Reconstruct a file from its shard INDEX
ConstructFromShards(shardIndexSCID string) -> {
    success: true,
    content: "...",
    originalSize: 28672,
    shardsUsed: 2,
...
| Suffix | Description |
|--------|-------------|
| `.shards` | A shard INDEX containing references to shard DOCs |
| `.shard` | An individual shard DOC (part of a larger file) |
| `.lib` | A library of reusable DOCs |
| `.bootstrap` | A collection of TELA apps/content for bootstrapping |

<Callout type="info">
  DocShards are transparent to the end user—the Browser automatically reconstructs sharded files. Use them when compression alone isn't enough to fit under the 18KB limit.
</Callout>
