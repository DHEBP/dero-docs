...
| Versioned SCID | `abc123...@txid456...` | Fetch specific version (see Version Control) |
...
- `TELA-STATIC`: Static files (images, SVG, fonts)
...
<Callout type="success">
  Static files (SVG, images, fonts) are now properly embedded in TELA content.
</Callout>

Static files are automatically inlined when referenced in HTML:

- **SVG files**: Embedded directly as inline SVG
- **Image files**: Converted to base64 data URIs
- **Automatic detection**: `<img src="...">` references are automatically resolved

### TELA V2 Contract Keys

<Callout type="success">
  Support for both TELA V1 and V2 contract formats.
</Callout>

TELA smart contracts store metadata (name, description, icon) in variable keys. The TELA specification evolved to support updatable headers, resulting in two header formats:

| Version | Variable Keys | Mutability | Source |
|---------|--------------|------------|--------|
| **V1 (Original)** | `nameHdr`, `descrHdr`, `iconURLHdr` | Immutable | Set at deployment only |
| **V2 (Enhanced)** | `var_header_name`, `var_header_description`, `var_header_icon` | Mutable | Can update with [TELA-MOD-1](/advanced-features/tela-mods) |

**Why Two Formats?**

According to the [official TELA specification](https://github.com/deroproject/tela-main), V2 headers were introduced to enable metadata updates:

- **V1 headers** (`nameHdr`, etc.) are immutable—once set during deployment, they cannot be changed
- **V2 headers** (`var_header_name`, etc.) are mutable—they can be updated later using TELA-MOD-1 Variable Store operations

**Example from TELA Spec:**
...
STORE("var_header_name", "My App")  // Can update via SetVar later
```
...
This ensures:
- **Backward compatibility** - Old V1 apps still work perfectly
- **Forward compatibility** - New V2 apps work correctly
- **No user action needed** - Hologram handles the format detection automatically
- **Seamless experience** - All TELA apps render regardless of deployment date
...
| **Wallet** | `GetAddress`, `GetBalance`, `Transfer`, `scinvoke`, etc. | ✅ Fully supported |
