...
The iframe sandbox grants `allow-forms` and `allow-modals` (modals and in-app forms are permitted; clipboard access is granted for TELA apps like Villager). External network isolation, not the sandbox, is what prevents content from reaching outside services.
...
RATE_LIMIT_MAX_HASHES = 500 per window
```
